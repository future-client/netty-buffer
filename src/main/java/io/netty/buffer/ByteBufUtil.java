...
public final class ByteBufUtil {
    private static final ThreadLocal<byte[]> BYTE_ARRAYS = new ThreadLocal<byte[]>() {
        @Override
    ...
    private static final int MAX_BYTES_PER_CHAR_UTF8 = (int) StandardCharsets.UTF_8.newEncoder()
            .onMalformedInput(CodingErrorAction.REPLACE)
            .onUnmappableCharacter(CodingErrorAction.REPLACE)
            .maxBytesPerChar();

    static final int WRITE_CHUNK_SIZE = 8192;
    static final ByteBufAllocator DEFAULT_ALLOCATOR = UnpooledByteBufAllocator.DEFAULT;
    ...
    private static int reserveAndWriteUtf8Seq(ByteBuf buf, CharSequence seq, int start, int end, int reserveBytes) {
        for (;;) {
            if (buf instanceof WrappedCompositeByteBuf) {
                ...
                byte[] bytes = seq.subSequence(start, end).toString().getBytes(StandardCharsets.UTF_8);
    ...
    public static int writeAscii(ByteBuf buf, CharSequence seq) {
        ...
        for (;;) {
            if (buf instanceof WrappedCompositeByteBuf) {
                ...
                byte[] bytes = seq.toString().getBytes(StandardCharsets.US_ASCII);
                buf.writeBytes(bytes);
                return bytes.length;
            }
        }
    ...
    static int writeAscii(AbstractByteBuf buffer, int writerIndex, CharSequence seq, int len) {
        ...
        // This is possible as we called ensureWritable(...) before.
        for (int i = 0; i < len; i++) {
            buffer._setByte(writerIndex++, c2b(seq.charAt(i)));
        }
        return len;
    }
